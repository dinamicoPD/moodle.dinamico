    ...
    for ($i = 1; $i <= $cantidad_grupos_ins; $i++){
        ...
        for ($j = $conteoInicial; $j <= $conteo; $j+=7){
            $colegios .= '<div class="input-group">';
                $colegios .= '<select name="Departamento_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $colegios .= '<option selected value="'.$array_ubicacion[$j].'">'.$array_ubicacion[$j+1].'</option>';
                $colegios .= '</select>';
                $colegios .= '<select name="Municipio_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $colegios .= '<option selected value="'.$array_ubicacion[$j+2].'">'.$array_ubicacion[$j+3].'</option>';
                $colegios .= '</select>';
                $colegios .= '<select name="Instituto_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $colegios .= '<option selected value="'.$array_ubicacion[$j+4].'">'.$array_ubicacion[$j+5].'</option>';
                $colegios .= '</select>';
        if ($array_ubicacion[$j+4] == 'otro'){     
                $colegios .= '<input name="Otro_'.$Preinscripcion["Id_preDocente"].'" type="text" class="form-control" readonly value="'.$array_ubicacion[$j+6].'">';           
        }                
            $colegios .= '</div>';
        }
        $colegios .= '<br>';
    ...
    for ($i = 1; $i <= $cantidad_grupos_ins; $i++){
        ...
        for ($j = $conteoInicial; $j <= $conteo; $j+=7){
            $cursos .= '<div class="input-group">';
                $cursos .= '<select name="Colegio_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $cursos .= '<option selected value="'.$array_grupos[$j].'">'.$array_grupos[$j+1].'</option>';
                $cursos .= '</select>';
                $cursos .= '<select name="Edicion_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $cursos .= '<option selected value="'.$array_grupos[$j+2].'">'.$array_grupos[$j+3].'</option>';
                $cursos .= '</select>';
                $cursos .= '<select name="Curso_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
                    $cursos .= '<option selected value="'.$array_grupos[$j+4].'">'.$array_grupos[$j+5].'</option>';
                $cursos .= '</select>';
                $cursos .= '<select name="Sigla_'.$j.'_'.$Preinscripcion["Id_preDocente"].'" class="form-select" disabled>';
